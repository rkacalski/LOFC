...
public class wishlistSelect : MonoBehaviour
{
    public static int cardPosinList;
    public static int selectedBinder;
    ...
    {
        selectedBinder = wishlistDropDown.chosenOption;

        if (selectedBinder.Equals(0))
        {
            tempCard = binder.oneStarbaseLight[cardPosinList];
        }
        if (selectedBinder.Equals(1))
        {
            tempCard = binder.oneStarbaseDark[cardPosinList];
        }
        if (selectedBinder.Equals(2))
        {
            tempCard = binder.oneStarbaseNuet[cardPosinList];
        }

        playerCards.wishlist.Insert(cardSlot, tempCard);
